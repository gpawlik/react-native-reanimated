...
- _gamma_ - Colors on web / mobile are expressed in sRGB colorspace which is gamma-corrected, that is non-linear.
  ...
  may appear darker than it should. That's why when iterpolating we convert sRGB to linear space first and then convert
...
The widget below shows how the interpolation looks like for different colors, color spaces and options. It's interactive, so you can choose colors and options.
